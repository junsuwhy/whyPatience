...
await asyncTest(
  'Game validation file should contain required validation functions',
  async () => {
    if (!existsSync(gameValidationPath)) {
      throw new Error('Game validation service file does not exist');
    }
    ...
    for (const func of requiredFunctions) {
      if (!content.includes(func)) {
        throw new Error(
          `Function '${func}' is missing from game validation service`
        );
      }
    }
  }
...
await asyncTest(
  'Game validation service should have proper TypeScript types',
  async () => {
    if (!existsSync(gameValidationPath)) {
      throw new Error('Game validation service file does not exist');
    }
...
await asyncTest(
  'validateTableauMove should have correct function signature',
  async () => {
    if (!existsSync(gameValidationPath)) {
      throw new Error('Game validation service file does not exist');
    }
    ...
    if (
      !content.includes('validateTableauMove') ||
      !content.includes('function') ||
      !content.includes('boolean')
    ) {
      throw new Error(
        'validateTableauMove function signature is incorrect or missing'
      );
    }
  }
);
...
await asyncTest(
  'validateFoundationMove should have correct function signature',
  async () => {
    if (!existsSync(gameValidationPath)) {
      throw new Error('Game validation service file does not exist');
    }
...
await asyncTest(
  'isValidCardSequence should validate alternating colors and descending ranks',
  async () => {
    if (!existsSync(gameValidationPath)) {
      throw new Error('Game validation service file does not exist');
    }
...
await asyncTest(
  'canPlaceOnFoundation should validate ascending same-suit sequence',
  async () => {
    if (!existsSync(gameValidationPath)) {
      throw new Error('Game validation service file does not exist');
    }
...
await asyncTest(
  'canPlaceOnTableau should validate descending alternate-color sequence',
  async () => {
    if (!existsSync(gameValidationPath)) {
      throw new Error('Game validation service file does not exist');
    }
...
await asyncTest(
  'isGameWon should check if all cards are in foundation piles',
  async () => {
    if (!existsSync(gameValidationPath)) {
      throw new Error('Game validation service file does not exist');
    }
...
await asyncTest(
  'validateStockPileOperation should validate stock pile interactions',
  async () => {
    if (!existsSync(gameValidationPath)) {
      throw new Error('Game validation service file does not exist');
    }
...
await asyncTest(
  'validateGameState should validate complete game state integrity',
  async () => {
    if (!existsSync(gameValidationPath)) {
      throw new Error('Game validation service file does not exist');
    }
...
await asyncTest(
  'Validation functions should have JSDoc documentation',
  async () => {
    if (!existsSync(gameValidationPath)) {
      throw new Error('Game validation service file does not exist');
    }
...
await asyncTest(
  'Validation functions should include proper error handling',
  async () => {
    if (!existsSync(gameValidationPath)) {
      throw new Error('Game validation service file does not exist');
    }
