...
describe('T022 GameState Model', () => {
  ...
  describe('Game Victory Detection', () => {
    ...
    test('should detect game won when all foundations complete', () => {
      ...
      gameState.foundation.forEach((pile, index) => {
        const suit = [Suit.HEARTS, Suit.DIAMONDS, Suit.CLUBS, Suit.SPADES][
          index
        ];
        pile.suit = suit;
        pile.topRank = Rank.KING;
        pile.cards = Array.from(
          { length: 13 },
          (_, i) => new Card(suit, (i + 1) as Rank, true)
        );
      });
    ...
    test('should detect game not won with partial foundations', () => {
      ...
      gameState.foundation[0].cards = Array.from(
        { length: 10 },
        (_, i) => new Card(Suit.HEARTS, (i + 1) as Rank, true)
      );
  ...
  describe('Cloning and Serialization', () => {
    ...
    test('should serialize and deserialize correctly', () => {
      ...
      expect(restored.statistics.moveCount).toBe(
        gameState.statistics.moveCount
      );
    ...
    test('should handle empty game serialization', () => {
      ...
      expect(restored.foundation.every(pile => pile.cards.length === 0)).toBe(
        true
      );
    });
